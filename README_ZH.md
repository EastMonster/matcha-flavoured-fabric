...
- 原版载体物品与 Matcha 物品彼此独立，只有 Matcha 的配方、掉落和交易会产出对应的 mod 物品。
- 与物品设计无关的载体副作用不会保留。例如奶酪不会毒害鹦鹉，也不能堆肥等。
- 精金套装 Divinity 统计到五件时使用四件套的最高加成；原数据包只定义了一至四件，五件时会落入无效果空档。
- 欢乐恶魂号角会选择 80 格内最近的欢乐恶魂；原数据包会任意选择一个匹配实体。
