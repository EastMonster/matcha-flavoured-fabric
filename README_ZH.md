...
- 不再把原版物品魔改成新物品，而是在 `matcha` 命名空间下独立注册装备、食物、鱼、祝福、行为物品和唱片等大部分内容。新增内容因此在物品栏及 JEI、Jade 等工具中更容易识别和查找；物品 ID 和指令与数据包版不同，**已有的数据包版物品也不会自动转换**。已按用途把注册物品放入对应的原版创造模式标签页。
